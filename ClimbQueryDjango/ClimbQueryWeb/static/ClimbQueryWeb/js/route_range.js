...
var RouteListBox = React.createClass({
    ...
    getInitialState: function() {
        return {ranges: [], crags: {}, responsiveSetting: 0, currentRange: 0};
    },
    ...
    _onSearch: function(){
        ...
        this._loadRangesFromServer(searchOptions);
    },
    _loadRangesFromServer: function(searchOptions) {
        $.ajax({
            method: "POST",
    ...
    _nextRange: function(){
        ...
        if (this.state.ranges.length > 0){
            newRangeIndex = (this.state.currentRange + 1)%this.state.ranges.length;
        }
        this.setState({currentRange: newRangeIndex});
    },
    _previousRange: function(){
        var newRangeIndex = 0
        if (this.state.ranges.length > 0){
            newRangeIndex =(this.state.currentRange + this.state.ranges.length - 1)%this.state.ranges.length;
        }
        this.setState({currentRange: newRangeIndex});
    },
    ...
    render: function() {
        ...
        var routes = [];
        if (this.state.ranges.length > 0){
            routes = this.state.ranges[this.state.currentRange].routes;
        }
        ...
        return (
            <div className="routeListBox">
                ...
                <ReactButton
                    ...
                    text="Previous"
                    onClick={this._previousRange}
                />
                <span><b>&nbsp;&nbsp;Range {this.state.currentRange+1}&nbsp;&nbsp;</b></span>
                <ReactButton
                    className="btn btn-default" 
                    text="Next"
                    onClick={this._nextRange}
                />
                <h1>Routes</h1>
                <RouteList
                    routes = {routes}
...
React.render(
    <RouteListBox urlRoutes="/ClimbQueryService/RouteRange/" urlCrags="/ClimbQueryService/Crag/"/>,
