def isValidRoute(route, max_route, min_route, min_stars):
    if route.stars >= min_stars and min_route <= route.grade <= max_route:
        return True
    return False
...
def findRoutes(route_interface, max_route=1000, min_route=0, min_stars=0):
    # input = open("Boven.txt", "r")
    all_routes = route_interface.get_routes()
    routes = []
    for route in all_routes:
        if isValidRoute(route, max_route, min_route, min_stars):
            routes += [route]

    return routes


def findRouteRanges(route_interface, range_length, required_grades, crag_requirements, star_requirements=None,
                    remove_non_valid=False):
