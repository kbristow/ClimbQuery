...
def clean_ranges(ranges, ranges_valid_routes):
    
    for i in range(len(ranges_valid_routes)-1, 0, -1):
        valid = False
        base_index = i
        compare_index = i-1
        if len(ranges_valid_routes[base_index]) > len(ranges_valid_routes[compare_index]):
            base_index = i - 1
            compare_index = i
        for route in ranges_valid_routes[base_index]:
            if not route in ranges_valid_routes[compare_index]:
                valid = True
                break
        if not valid:
